...
int n;
vector<string> ans;
void recursion(string s,int open,int close){
    if(open == close && open == n){
        ans.push_back(s);
        return;
    }
    if(open == close) {
        recursion(s+"(",open+1,close);
    }else if(open > close){
        if(open < n)  recursion(s+"(",open+1,close);
        if(close < n) recursion(s+")",open,close+1);
    }
}

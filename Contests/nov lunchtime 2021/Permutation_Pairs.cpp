...
vector<vector<int>> vecs[11];
void permute(vector<int> a, int l, int r,int idx)
{
    // Base case
    if (l == r)
        vecs[idx].pb(a);
...
ll fun(vector<int> vec,int n,int k){
    ll cnt= 0;
    for(int i=0;i<n;i++){
...
void solve(){
    ...
    if(vecs[n].empty()) permute(v,0,n-1,n);
    ll ans = 0;
    for(auto vec: vecs[n]){
        ans = (ans + fun(vec,n,k)) % mod;
    }
    memo[n][k] = ans;
    cout << ans << '\n';
}

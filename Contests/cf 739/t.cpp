...
void solve(){
    ...
    string ans = "";
    set<char> s;
    while(s.size() <= k && i < len){
        if(s.find(sn[i]) != s.end() || s.size() < k){
            ans.pb(sn[i]);
            s.insert(sn[i]);
            mn = min(mn,sn[i]);
            mx = max(mx,sn[i]);
    ...
    if(s.upper_bound(sn[i]) != s.end()){
        char ch = *(s.upper_bound(sn[i]));
        ans.pb(ch);
        ans = ans + string(len-ans.length(),mn);
        cout << ans << '\n';
        return;
    }
    if(sn[i-1] != mx){
        ans.pop_back();
        ans.push_back(*s.upper_bound(sn[i-1]));
        ans = ans + string(len-ans.length(),mn);
        cout << ans << '\n';
        return;
    }

...
int distance(int row,int col,char p,vector<vector<bool>> vis={}){
    if(memo[row][col] != -1) return memo[row][col];
    if(row == n && col == m){
        memo[row][col] = 0;
        return 0;
    }
    vector<int> tblr(4,INT_MAX);
    if(row > 1 && mat[row-1][col] != '#' && p != 'b'){
        tblr[0] = distance(row-1,col,'t') + 1;
    }
    if(row < n && mat[row+1][col] != '#' && p != 't'){
        tblr[1] = distance(row+1,col,'b') + 1;
    }
    if(col < m && mat[row][col+1] != '#' && p != 'r'){
        tblr[2] = distance(row,col+1,'l') + 1; 
    }
    if(col > 1 && mat[row][col-1] != '#' && p != 'l'){
        tblr[3] = distance(row,col-1,'r') + 1;
    }

    int mn = *min_element(all(tblr));
    memo[row][col] = mn;
    return mn;
}
void solve(){
    cin >> n >> m >> k;
    mat.resize(n+1,string(m+1,'x'));

...
void _out(set<int> s) {for(auto i:s) cout<<i<<' '; cout<<nl;}
...
void solve(){
    ...
    for(int i=0;i<n;i++){
        ...
        for(int p: pf[i]){
            if(primes.find(p) == primes.end()) ok = 1;
        }
        if(!ok){
            for(int p: pf[i]) primes.erase(p);
            cout << arr[i] << ' ';
            continue;
        }
        int x = arr[i]+1;
        while(1){
            auto v = primeFators(x);
            bool pos = 1;
            for(int j: v) {
                if(primes.find(j) == primes.end()) pos = 0;
            }
            if(!pos){
                x++;
                continue;
            }
            for(int j: v) primes.erase(j);
            break;
        }
        cout << x << ' ';
    }
